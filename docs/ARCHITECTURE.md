...
We use three static memory pools, one for the input line, another one for the Abstract Syntax Tree and command environment and the third one for local and environment variables lists and command. They all have a semi-arbitrary size, given that there are no bottlenecks in other places of the program that limit the functionality of the shell. For example, there's no limit of commands in one prompt for the mandatory part, or there’s no limit for the number of redirections of a command, so in principle you can redirect until you fill the RAM with the line buffer.
...
We use the same linked list of variables, with a flag to differnciate local from environment variables. theres no limit to the number of variables (local or environmental), or the size of each value. But execve only accepts approximately 2MB of arguments (args + envp), and we are going to use this size limit for the pool.
